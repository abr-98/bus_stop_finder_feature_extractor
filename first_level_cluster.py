...
class First_level_cluster():
    ...
    def process_line(self,raw_data,trail_number):
        ...
        line= raw_data.split(',')
        #print line
        latitude, longitude, date, timestamp = line[0],line[1], line[2], line[-1].split()[0]
        return latitude,longitude, date, timestamp,trail_number
    ...
    def get_zero_speed_data(self,trail_number):
        ...
        current_latitude, current_longitude, current_date, current_timestamp, trail_number= self.process_line(self.data_lines[0],trail_number)

        for next_line in self.data_lines[1:]:
            #get the next point
            next_latitude, next_longitude, next_date, next_timestamp,trail_number= self.process_line(next_line,trail_number)
            ...
            #     count+=1
            if (get_spherical_distance(current_latitude,next_latitude,current_longitude,current_longitude)<0.1):
                count+=1
            else:
                ...
                if count>0:
                    ...
                    t = [int(i) for i in current_timestamp.split(':')]
                    seconds1=int(datetime.timedelta(hours=int(t[0]),minutes=int(t[1]),seconds=int(t[2])).total_seconds())
                    t= [int(i) for i in next_timestamp.split(':')]
                    seconds2=int(datetime.timedelta(hours=int(t[0]),minutes=int(t[1]),seconds=int(t[2])).total_seconds())
                    count=seconds2-seconds1-1
                    self.zero_speed_data.append([current_latitude,current_longitude,current_date,current_timestamp,count,trail_number])
    ...
    def get_local_group_leaders(self):
        ...
        for each_point in self.local_group_data:
            ...
            if each_point[-1] == group_number:
                ...
                # group leader contains two extra field denoting the time stamp of first local leader in the group and ending time for the whole group
                # group_leader= [latitude,longitude,date,timestamp,count,local_group_number,group_starting_time,group_ending_time]
                # group_leader= [latitude,longitude,date,timestamp,count,local_group_number]
...
def main(directory,DISTANCE_THRESHOLD,TIME_START,TIME_END,TRAIL_ID_RANGE):
    ...
    if 'output_data' not in os.listdir('.'):
        ...
        # clean_directory('output_data')
        # print "output_data dir cleaned"
        pass
        
    data_sub_folder=directory.split('/')[-1]

    if data_sub_folder not in os.listdir('./output_data'):
        os.mkdir('./output_data/'+data_sub_folder)
        # print "ksdfs"
    else:
        clean_directory('output_data/'+data_sub_folder)
    ...
    for trail in trails:
        ...
        # print call_algo.data_lines
        call_algo.get_zero_speed_data(trail_index)  #get all the zero-speed points and store it in zero_speed_data[]
        header= "latitude,longitude,date,time-stamp,count,trail_number"
        write_data_to_file('output_data/'+data_sub_folder+'/'+trail+'_zero_speed.csv',call_algo.zero_speed_data,header)

        call_algo.get_local_groups(DISTANCE_THRESHOLD)     #get all local groups from zero-speed points and store in local_group_data[]
        header= "latitude,longitude,date,time-stamp,count,trail_number,local_group_number"
        write_data_to_file('output_data/'+data_sub_folder+'/'+trail+'_local_groups.csv',call_algo.local_group_data,header)

        call_algo.get_local_group_leaders() #get all local group leaders from local_group_data and store in local_group_leader[]
        header= "latitude,longitude,date,time-stamp,total_waiting_time,trail_number,local_group_number,group_starting_time,group_ending_time"
        write_data_to_file('output_data/'+data_sub_folder+'/'+trail+'_local_group_leader.csv',call_algo.local_group_leader,header)
