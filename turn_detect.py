...
def turn():
    ...
    signal=[]
    name='feature.csv'
    name2='turn/turn_1.csv'
    name3='turn/turn_2.csv'
    df=pd.read_csv(name)
   ...
   # k=int(k)
    while k<l1:
        if 'Turn' in str(df1.iloc[k]['Turn']):
            print(str(df1.iloc[k]['Turn']))
            j=0
            while j<l:
                if get_spherical_distance(df.iloc[j]['latitude'],df1.iloc[k]['#lat'],df.iloc[j]['longitude'],df1.iloc[k]['long'])<20:
                    if signal1[j]==0:
                        signal1[j]=1
    ...
    k=0
    while k<l2:
        if 'Turn' in str(df2.iloc[k]['Turn']):
            print(str(df2.iloc[k]['Turn']))
            j=0
            while j<l:
                if get_spherical_distance(df.iloc[j]['latitude'],df2.iloc[k]['#lat'],df.iloc[j]['longitude'],df2.iloc[k]['long'])<20:
                    if signal2[j]==0: 
                        signal2[j]=1
